...
export default function Plan() {
  const [tasks, setTasks] = useState<Task[]>(initialTasks.map(task => ({
    ...task,
    status: "pending", // Reset main task
    subtasks: task.subtasks.map(sub => ({ ...sub, status: "pending" })) // Reset subtasks
  })));
  ...
  useEffect(() => {
    if (isAutoRunning) {
      timerRef.current = setInterval(() => {
        setTasks((prevTasks) => {
          // 1. Check if a failure already exists in the system
          const hasFailure = prevTasks.some(t =>
            t.status === "failed" || t.subtasks.some(s => s.status === "failed")
          );
          ...
          const activeTaskIndex = newTasks.findIndex(t => t.status !== "completed");
          ...
          if (!expandedTasks.includes(activeTask.id)) {
            setExpandedTasks(prev => [...prev, activeTask.id]);
          }

          const activeSubtaskIndex = activeTask.subtasks.findIndex(s => s.status !== "completed");

          if (activeSubtaskIndex !== -1) {
            const subtasks = [...activeTask.subtasks];
            const currentSub = { ...subtasks[activeSubtaskIndex] };

            if (currentSub.status === "pending") {
              currentSub.status = "in-progress";
              activeTask.status = "in-progress";
            } else if (currentSub.status === "in-progress") {
              // --- FAILURE LOGIC ---
              // 10% chance to fail for demonstration
              ...
              if (shouldFail) {
                ...
                // Set all subsequent tasks and subtasks to failed
            ...
            subtasks[activeSubtaskIndex] = currentSub;
                        ...
                        <AnimatePresence mode="wait">
                          ...
                          >
                            {task.status === "completed" ? (
                              ...
                              <OrbitalLoader className="h-4.5 w-4.5 text-blue-500" />
